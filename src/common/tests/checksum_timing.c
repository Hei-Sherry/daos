	    ...
	    const int sizes_count, uint32_t iterations)
{
	...
	for (size_idx = 0; size_idx < sizes_count; size_idx++) {
		...
		for (type_idx = 0; type_idx < types_count; type_idx++) {
			...
			args.iterations = 1; /** run once so rest can be cached */
...
#ifdef ISAL_CRYPTO_SUPPORT
		csum_fts[type_count++] = &sha1_algo;
		csum_fts[type_count++] = &sha256_algo;
		csum_fts[type_count++] = &sha512_algo;
#endif
	}
...
//		printf("## Iterations: %d ##\n", i);
//		rc = run_timings(csum_fts, type_count, sizes, sizes_count, i);
//		printf("\n");
//
//	}
