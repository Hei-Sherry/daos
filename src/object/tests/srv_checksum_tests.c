...
static int fake_reset(struct daos_csummer *obj)
{
	fake_update_buf[0] = '>';
	fake_update_buf++;
	fake_update_bytes++;
...
/* [todo-ryon]: for commit message:
 * Changed checking specific csum values, just comparing whole csum buffer to
 * what's expected. Easier to read because it's a string. Can also see what
 * parts of the csum buffer were copied vs new checksum.
 * */
#define ASSERT_CSUM(ctx, csum) \
	assert_memory_equal(csum, ctx.iod_csum->ic_data->cs_csum, \
		sizeof(csum) - 1);
#define ASSERT_CSUM_EMPTY(ctx, idx) \
	assert_string_equal("", ctx.iod_csum->ic_data->cs_csum + (idx * ctx.iod_csum->ic_data->cs_len));
#define ASSERT_CSUM_IDX(ctx, csum, idx) \
	assert_memory_equal(csum, ctx.iod_csum->ic_data->cs_csum + (idx * ctx.iod_csum->ic_data->cs_len), \
		sizeof(csum) - 1);
...
request_that_matches_single_extent(void **state)
{
	...
	memset(ctx.iod_csum->ic_data->cs_csum, 0, ctx.iod_csum->ic_data->cs_buf_len);
...
fetch_with_hole(void **state)
{
	...
	ARRAY_TEST_CASE_CREATE(&ctx, {
		...
		.layout = {
			{.data = "ABCD", .sel = {0, 3}, .ful = {0, 3} },
			{.data = "", .sel = {4, 5}, .ful = {4, 5}, .is_hole = true},
...
fetch_with_hole2(void **state)
{
	...
	ARRAY_TEST_CASE_CREATE(&ctx, {
		...
		.layout = {
			{.data = "ABC", .sel = {0, 2}, .ful = {0, 2} },
			{.data = "", .sel = {3, 5}, .ful = {3, 5}, .is_hole = true},
...
fetch_with_hole3(void **state)
{
	...
	ARRAY_TEST_CASE_CREATE(&ctx, {
		...
		.layout = {
			{.data = "A", .sel = {0, 0}, .ful = {0, 0} },
			{.data = "", .sel = {1, 1}, .ful = {1, 1}, .is_hole = true},
			{.data = "B", .sel = {2, 2}, .ful = {2, 2} },
			{.data = "", .sel = {3, 3}, .ful = {3, 3}, .is_hole = true},
			{.data = "C", .sel = {4, 4}, .ful = {4, 4} },
			{.data = "", .sel = {5, 5}, .ful = {5, 5}, .is_hole = true},
			{.data = "D", .sel = {6, 6}, .ful = {6, 6} },
			{.data = "", .sel = {7, 7}, .ful = {7, 7}, .is_hole = true},
			{.data = "E", .sel = {8, 8}, .ful = {8, 8} },
			{.data = "", .sel = {9, 9}, .ful = {9, 9}, .is_hole = true},
...
fetch_with_hole4(void **state)
{
	...
	ARRAY_TEST_CASE_CREATE(&ctx, {
		...
		.rec_size = 1,
		.layout = {
			{.data = "", .sel = {0, 7}, .ful = {0, 7}, .is_hole = true},
			{.data = "ABCDEF", .sel = {8, 13}, .ful = {8, 13} },
			{.data = "", .sel = {14, 17}, .ful = {14, 17}, .is_hole = true},
...
fetch_with_hole5(void **state)
{
	...
	ARRAY_TEST_CASE_CREATE(&ctx, {
		...
		.layout = {
			...
			{.data = "", .sel = {3, 8}, .ful = {3, 8}, .is_hole = true},
...
static const struct CMUnitTest array_tests[] = {
	...
	TA("SRV_CSUM_ARRAY02: Whole extent requested, broken into multiple chunks",
	   request_that_matches_single_extent_multiple_chunks),
	TA("SRV_CSUM_ARRAY03: Whole extent requested, broken into multiple chunks, not aligned to chunk",
	...
	TA("SRV_CSUM_ARRAY13: Partial Extents, extent smaller than chunk", extent_smaller_than_chunk),
	TA("SRV_CSUM_ARRAY14: Extent is larger than chunk", extent_larger_than_request),
	TA("SRV_CSUM_ARRAY15: Full and partial chunks", fetch_multiple_unaligned_extents),
