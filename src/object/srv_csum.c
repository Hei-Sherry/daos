...
struct csum_context {
	...
	/** If the chunk is a power of two, can use more efficient math */
	bool			 cc_chunk_is_p2;
	...
	daos_size_t size)
{
	...
	ctx->cc_rec_chunksize = daos_csummer_get_rec_chunksize(csummer, size) /
									size;
	ctx->cc_chunk_is_p2 = is_p2(ctx->cc_rec_chunksize);
...
static daos_size_t
cc_round_up_to_chunk(const struct csum_context *ctx, daos_size_t n)
{
	return csum_round_up_to_chunk(n, ctx->cc_rec_chunksize,
				      ctx->cc_chunk_is_p2);
}
static daos_size_t
cc_round_down_to_chunk(const struct csum_context *ctx, daos_size_t n)
{
	return csum_round_down_to_chunk(n, ctx->cc_rec_chunksize,
					ctx->cc_chunk_is_p2);
}
...
static bool
cc_is_aligned(struct csum_context *ctx, daos_off_t idx)
{
	return idx % ctx->cc_rec_chunksize == 0;
}
...
cc_need_new_csum(struct csum_context *ctx, daos_off_t idx)
{
	...
	/* [todo-ryon]: ... */
	uint64_t		 cur_idx; // = ctx->cc_cur_recx_idx;
	cur_idx = idx;
	 ...
	 */
	if (br->br_has_prefix &&
	    cc_in_same_chunk(ctx, cur_idx, br->br_raw.dcr_lo))
		return true;
	/**
	 ...
	 */
	if (br->br_has_suffix &&
	    cc_in_same_chunk(ctx, cur_idx, br->br_raw.dcr_hi))
		return true;

	/** another extent in the same chunk */
	if (cc_next_non_hole_extent_in_chunk(ctx, cur_idx))
		return true;
...
void cc_skip_hole(struct csum_context *ctx)
{
	...
	daos_size_t lo;
	daos_size_t hi;
	daos_size_t lo_aligned_up;
	daos_size_t hi_aligned_down;

	lo = ctx->cc_biov_ranges.br_req.dcr_lo;
	hi = ctx->cc_biov_ranges.br_req.dcr_hi;

	lo_aligned_up = cc_round_up_to_chunk(ctx, lo);
	hi_aligned_down = cc_round_down_to_chunk(ctx, hi + 1) - 1;

	/**
	 * lo aligned could be higher than hi aligned if both are in the same chunk
	 * hi can be less than hi_aligned_down if it wraps below 0
	 */
	if (lo_aligned_up > hi_aligned_down || hi_aligned_down > hi)
		csum_nr = 0;
	else
		csum_nr = (hi_aligned_down - lo_aligned_up + 1) /
...
cc_create(struct csum_context *ctx)
{
	...
	do {
		...
		if (bio_addr_is_hole(&cc2biov(ctx)->bi_addr)) {
			/** will skip until end of biov or end of chunk */
...
cc_copy(struct csum_context *ctx)
{
	 ...
	 */
	daos_size_t csum_nr = ctx->cc_biov_ranges.br_req.dcr_hi / ctx->cc_rec_chunksize -
...
//			D_PRINT("[RYON] %s:%d [%s()] > Creating new checksum for biov: %d\n", __FILE__, __LINE__, __FUNCTION__, ctx->cc_bsgl_idx.iov_idx);
			rc = cc_create(ctx);
		} else {
//			D_PRINT("[RYON] %s:%d [%s()] > Copy checksum for biov: %d\n", __FILE__, __LINE__, __FUNCTION__, ctx->cc_bsgl_idx.iov_idx);
...
//			D_PRINT("[RYON] %s:%d [%s()] > elapsed: %fus\n", __FILE__, __LINE__, __FUNCTION__, d_time2us(d_time_elapsed(start_time)));
			if (rc != 0) {
